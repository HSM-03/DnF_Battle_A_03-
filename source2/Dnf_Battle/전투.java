...
public class 전투 {
    ...
    // [Phase 1] 몬스터공격
    // ────────────────────────────────────────────
    public void 몬스터공격(String 플레이어id, 캐릭터 캐릭터) {
        if (!플레이어.플레이어체크(플레이어id)) {
            System.out.println(">> 인증 실패: 올바른 플레이어가 아닙니다.");
            return;
        }
        ...
        System.out.printf(">> %s 공격! 데미지 = %.1f | 등급 = %s%n",
                캐릭터.get캐릭터명(), 데미지, 등급);
    }
    ...
    // ────────────────────────────────────────────
    public boolean 아이템획득(String 플레이어id, String 아이템명, String 타입, int 가치) {
        if (!플레이어.플레이어체크(플레이어id)) {
            System.out.println(">> 인증 실패: 올바른 플레이어가 아닙니다.");
            return false;
        }

        캐릭터 캐릭터 = 캐릭터맵.get(플레이어id);
        if (캐릭터 == null) {
            System.out.println(">> 아이템을 획득할 캐릭터가 존재하지 않습니다.");
            return false;
        }

        인벤토리 인벤토리 = 캐릭터.get인벤토리();
        if (인벤토리.가득찼는가()) {
            System.out.printf(">> 인벤토리가 가득 찼습니다(%d/%d). '%s' 획득 실패.%n",
                    인벤토리.현재개수(), 인벤토리.get최대용량(), 아이템명);
            return false;
        }
        ...
        if (성공) {
            System.out.printf(">> 아이템 획득: %s | 인벤토리(%d/%d)%n",
                    새아이템.toString(), 인벤토리.현재개수(), 인벤토리.get최대용량());
        }
        return 성공;
    }
    ...
    public boolean 길드가입(String 플레이어id, String 길드명) {
        ...
        if (성공) {
            System.out.printf(">> '%s' 캐릭터가 길드 '%s'에 가입했습니다. 길드 인원(%d/%d)%n",
                    캐릭터.get캐릭터명(), 길드.get길드명(), 길드.현재인원(), 길드.get최대인원());
        }
        return 성공;
    }
    ...
    public void 길드등록(길드 길드) {
        길드맵.put(길드.get길드명(), 길드);
    }

    public 캐릭터 get캐릭터(String 플레이어id) {
